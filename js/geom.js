...
let root = null, torusRadius;



function createSceneA() {
    torusRadius = 15;
    root = starburstsOnSphereA(400, torusRadius, 100, 1)
    scene.add(root);
}


function starburstsOnSphereA(nbrBursts, torusRadius, maxRays, maxRad) {
    let root = new THREE.Object3D();
    for (let i = 0; i < nbrBursts; i++) {
        let mesh = starburstA(maxRays, maxRad);
        let p = getRandomPointOnSphere(torusRadius);
...
function update() {
    ...
    if (root)
        scene.remove(root);
    root = starburstsOnSphereA(nbrBursts, torusRadius, maxRays, burstRadius);
    scene.add(root);
}
